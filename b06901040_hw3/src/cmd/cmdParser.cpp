...
CmdParser::parseCmd(string& option)
{
   ...
   //part1
   string cmd ="";
   unsigned leading_length = 0;
   while(true)
   {
      if(str[leading_length] == 0 || str[leading_length] == ' ')
         break;
      cmd.push_back(str[leading_length]);
      leading_length++;
   }
   for(auto i = _cmdMap.begin(); i != _cmdMap.end(); i++)
   {
      string Cmpstr;
      Cmpstr.append(i -> first);
      Cmpstr.append(i -> second -> getOptCmd());
      if(myStrNCmp(Cmpstr, cmd,i -> first.size() ) == 0)
      {
         if (str[leading_length] !=0)
            leading_length++;
         while(true)
         {
            if(str[leading_length] == 0)
               break;
            option.push_back(str[leading_length]);
            leading_length++;
         } 
         return i -> second;
      }
