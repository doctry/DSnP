...
private:
   ...
   MemRecycleList<T>* getMemRecycleList(size_t n) {
      size_t m = n % R_SIZE;
      // TODO
      MemRecycleList<T>* temp;
      temp = &_recycleList[m];
      while(temp -> _arrSize != n)
      {
        if(temp -> _nextList == 0)
           {
              MemRecycleList<T>* temp_1 = temp;
              temp = new MemRecycleList<T>(n);
              temp_1 -> _nextList = temp;
              break;
           }
        else
           temp = temp -> _nextList;
   ...
   // t is the #Bytes reqw  uested from new or new[]
        ...
        if(!_activeBlock -> getMem(t,ret))
           {
              ...
              else if(_activeBlock -> getRemainSize() < S)
                 ;
              else
                 getMemRecycleList(getArraySize(_activeBlock -> getRemainSize())) -> pushFront((T*)(_activeBlock -> _ptr));
              //new Block
